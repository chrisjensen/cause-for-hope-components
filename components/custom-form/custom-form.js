	...
	class FormStep extends React.Component {
		...
		buttons = () => {
			const nextText = this.props.shouldSave() ?
				this.props.actionText || 'Save' : 'Next';
	...
	class CustomForm extends React.Component {
		...
		buildSteps(steps) {
			...
			const builtSteps = steps.map((step, index) => {
				...
				let actionText = (index === steps.length - 1) ? 'Save' : null;

				return props => <Component actionText={actionText} {...props} {...stepProps} pageIndex={index} />;
			});
